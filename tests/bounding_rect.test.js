...
describe('bounding rects', () => {
  test('it can intersect with overlapping rects', () => {
    const boxA = createBoundingRect({ x: 0, y: 0, width: 10, height: 10 })
    const boxB = createBoundingRect({ x: 9, y: 9, width: 5, height: 5 })
    expect(boxA.intersects(boxB)).toBe(true)
  })

  test('it can intersect with enveloped rects', () => {
    const boxA = createBoundingRect({ x: 0, y: 0, width: 10, height: 10 })
    const boxB = createBoundingRect({ x: 3, y: 3, width: 2, height: 2 })
    expect(boxA.intersects(boxB)).toBe(true)
  })

  test('it can intersect touching rects', () => {
    const boxA = createBoundingRect({ x: 0, y: 0, width: 10, height: 10 })
    const boxB = createBoundingRect({ x: 10, y: 0, width: 10, height: 10 })
    expect(boxA.intersects(boxB)).toBe(true)
  })

  test('it correctly handles non-intersecting rects', () => {
    const boxA = createBoundingRect({ x: 0, y: 0, width: 10, height: 10 })
    const boxB = createBoundingRect({ x: 11, y: 0, width: 10, height: 10 })
    expect(boxA.intersects(boxB)).toBe(false)
  })

  test('it correctly determines if a point is contained inside', () => {
    const box = createBoundingRect({ x: 0, y: 0, width: 10, height: 10 })
  ...
  test('it can center to another rect', () => {
    const boxA = createBoundingRect({ x: 0, y: 0, width: 8, height: 8 })
    const boxB = createBoundingRect({ x: 0, y: 0, width: 4, height: 4 })
    boxB.centerTo(boxA)
